...
export default function EventDashboardClient({ slug }: { slug: string }) {
  ...
  const saveWalkin = async () => {
    ...
    const { data, error } = await supabase.from('event_guests')
      .insert([{ event_id: event.id, guest_name: walkinName.trim(), phone: walkinPhone.trim() || null, guest_count: walkinCount, checked_in: true, checked_in_at: new Date().toISOString() }])
      .select('*').single()
    setSavingWalkin(false)
    if (!error && data) {
      setGuests(prev => [...prev, data as Guest].sort((a, b) => a.guest_name.localeCompare(b.guest_name)))
      setWalkinOpen(false); setWalkinName(''); setWalkinPhone(''); setWalkinCount(1)
    }
  }
              ...
              <div key={g.id} style={{ background: '#fff', borderRadius: 12, padding: '12px 14px', boxShadow: '0 2px 10px rgba(0,0,0,0.04)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11.5, color: '#94a3b8', marginTop: 2 }}>{g.phone || 'no phone'} · Code {g.id.slice(0, 8).toUpperCase()}</div>
                  </div>
                </div>
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 20, width: '100%', maxWidth: 360 }}>
            ...
            {scannedGuest && scannedGuest !== 'not-found' && (
              <div style={{ padding: '6px 0' }}>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#0f2438' }}>{scannedGuest.guest_name}{scannedGuest.guest_count > 1 ? ` (+${scannedGuest.guest_count - 1})` : ''}</div>
                <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 14 }}>{scannedGuest.phone || 'no phone'}</div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                  <button onClick={() => { toggleCheckIn(scannedGuest); setScannedGuest({ ...scannedGuest, checked_in: !scannedGuest.checked_in }) }} style={{
                    flex: 1, padding: '11px 10px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 12.5, fontWeight: 700,
                    background: scannedGuest.checked_in ? '#16a34a1a' : ACCENT, color: scannedGuest.checked_in ? '#16a34a' : '#fff',
                  }}>{scannedGuest.checked_in ? `✓ Checked In · ${fmtTime(scannedGuest.checked_in_at)}` : 'Check In'}</button>
                  <button onClick={() => { toggleMeal(scannedGuest); setScannedGuest({ ...scannedGuest, meal_claimed: !scannedGuest.meal_claimed }) }} style={{
                    flex: 1, padding: '11px 10px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 12.5, fontWeight: 700,
                    background: scannedGuest.meal_claimed ? '#c9a2271a' : ACCENT_LIGHT, color: scannedGuest.meal_claimed ? '#946f00' : '#1c1400',
                  }}>{scannedGuest.meal_claimed ? `✓ Meal Given · ${fmtTime(scannedGuest.meal_claimed_at)}` : 'Mark Meal'}</button>
                </div>
