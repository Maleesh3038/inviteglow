...
// ── Floral arch corner motif ──
function ArchFlorals({ color, size = 70, flip = false }: { color: string; size?: number; flip?: boolean }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" style={{ transform: flip ? "scaleX(-1)" : undefined, display: "block" }}>
      <path d="M10 90 C20 60 15 30 40 10" stroke={color} strokeWidth="2" fill="none" opacity="0.55" />
      {[20, 38, 55, 72].map((y, i) => (
        <ellipse key={i} cx={14 + i * 6} cy={y} rx="8" ry="5" fill={color} opacity={0.5 + i * 0.1}
          transform={`rotate(${-40 + i * 12} ${14 + i * 6} ${y})`} />
      ))}
    </svg>
  )
}

// ── Guest intro screen — warm golden-garden shimmer, "Dear [Name]," shown
// for ~5s before the cover. Only appears with ?name=..., toggle via
// couple.show_guest_intro (defaults to on). ──
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background: `linear-gradient(160deg, ${cream} 0%, #fbeedb 45%, ${cream} 100%)`,
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
        textAlign: "center", padding: "2rem", overflow: "hidden",
      }}>
      <div style={{ position: "absolute", top: 20, left: 10, opacity: 0.5 }}><ArchFlorals color={primary} size={90} /></div>
      <div style={{ position: "absolute", top: 20, right: 10, opacity: 0.5 }}><ArchFlorals color={primary} size={90} flip /></div>
      <div style={{ position: "absolute", width: 280, height: 280, borderRadius: "50%", background: `radial-gradient(circle, ${primaryLight}33, transparent)`, top: "24%", left: "50%", transform: "translateX(-50%)" }} />

      <motion.div initial={{ scale: 0.4, opacity: 0, rotate: -12 }} animate={{ scale: [0.4, 1.1, 1], opacity: 1, rotate: 0 }}
    ...
    <div style={{ display: "flex", justifyContent: "center", gap: 12, maxWidth: 380, margin: "0 auto" }}>
      {[["Days", t.d], ["Hours", t.h], ["Mins", t.m], ["Secs", t.s]].map(([l, v]) => (
        <div key={l} style={{ flex: 1, textAlign: "center" }}>
          <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#fff", border: `1.5px solid ${primaryLight}`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 8px", boxShadow: `0 4px 16px ${primary}22` }}>
...
function MusicPlayerUI({ title, artist, audioRef, primary, primaryLight, dark, muted }: { title: string; artist: string; audioRef: React.RefObject<HTMLAudioElement | null>; primary: string; primaryLight: string; dark: string; muted: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, background: "#fdf6ec", borderRadius: 16, padding: 16 }}>
...
function RSVP({ coupleId, askDrinking, primary, dark, cream, muted, guestName }: { coupleId: string; askDrinking: boolean; primary: string; dark: string; cream: string; muted: string; guestName: string }) {
  ...
  return (
    <div style={{ background: `linear-gradient(135deg,#f0dcc0,${cream})`, padding: "40px 1.5rem", textAlign: "center" }}>
        ...
        {step === "form" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <button onClick={() => name.trim() && save("no", null, 1)} disabled={saving} style={{ padding: 13, borderRadius: 10, background: "transparent", color: muted, border: "1px solid #e8ddd0", cursor: "pointer", fontSize: 12, opacity: saving ? 0.6 : 1 }}>{saving ? "..." : "✗ Decline"}</button>
            </div>
          </>
        )}
        {step === "count" && (
...
function WishLightbox({ media, index, onIndex, onClose }: {
  ...
  return (
    <div onClick={onClose} style={{
      position: "fixed", inset: 0, background: "rgba(61,43,31,0.92)", zIndex: 500,
      ...
      {shown.map((m, idx) => {
        ...
        return (
          <div key={idx} onClick={() => onOpen(idx)} style={{
            ...
            {isMoreTile && (
              <div style={{ position: "absolute", inset: 0, background: "rgba(61,43,31,0.55)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 18, fontWeight: 700, zIndex: 2 }}>
                +{media.length - 4}
              </div>
            )}
          </div>
...
const sectionCard: React.CSSProperties = { background: "#fff", margin: "0 16px 16px", borderRadius: 22, padding: "1.8rem", boxShadow: "0 2px 20px rgba(0,0,0,0.05)", position: "relative" }
...
export default function GoldenGardenTemplate({ couple }: { couple: Couple }) {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", background: "#fdf6ec" }} />}>
      <GoldenGardenInner couple={couple} />
    </Suspense>
  )
}

function GoldenGardenInner({ couple }: { couple: Couple }) {
  ...
  return (
    <div style={{ fontFamily: "'Inter',sans-serif", minHeight: "100vh", background: "#fdf6ec" }}>
      <style>{`
        ...
        input::placeholder { color: #c9b090; }
      `}</style>
        ...
        <AnimatePresence>
          {!opened && (
            <motion.div key="cover" exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.5 }}
              ...
              <div style={{ position: "absolute", inset: 0, background: `linear-gradient(180deg, rgba(61,43,31,0.4) 0%, rgba(61,43,31,0.15) 35%, rgba(61,43,31,0.3) 65%, rgba(61,43,31,0.7) 100%)` }} />

              <div style={{ position: "absolute", top: 20, left: 12, animation: "sway 5s ease-in-out infinite", transformOrigin: "top" }}><ArchFlorals color={PRIMARY_LIGHT} size={80} /></div>
              <div style={{ position: "absolute", top: 20, right: 12, animation: "sway 5s ease-in-out infinite 1s", transformOrigin: "top" }}><ArchFlorals color={PRIMARY_LIGHT} size={80} flip /></div>
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}
                ...
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.85)", lineHeight: 1.7, marginBottom: "1.6rem", textShadow: "0 2px 10px rgba(0,0,0,0.4)" }}>
                  Join us under the garden arch as we<br />begin our forever together
                </div>
                ...
                <button onClick={handleOpen} style={{
                  ...
                  cursor: "pointer", fontFamily: "'Inter',sans-serif", fontWeight: 600, boxShadow: "0 8px 24px rgba(0,0,0,0.35)",
                }}>
                  Open Invitation →
                </button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
            ...
            <div style={{ position: "relative", height: 460, overflow: "hidden" }}>
              ...
              <img src={W.couplePhoto} alt={`${W.bride} and ${W.groom}`} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 20%" }}
                onError={e => { (e.currentTarget as HTMLImageElement).src = DEFAULT_PHOTO }} />
              <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top,rgba(253,246,236,1) 0%,rgba(61,43,31,0.15) 55%,rgba(61,43,31,0.4) 100%)" }} />
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0 1.5rem 24px", textAlign: "center" }}>
                ...
                <div style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic", fontSize: "clamp(2.4rem,8vw,3.6rem)", color: "#fff", lineHeight: 1, textShadow: "0 2px 20px rgba(61,43,31,0.3)" }}>
            ...
            {(W.brideFamilyName || W.groomFamilyName) && (
              <motion.div style={sectionCard} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <div style={{ textAlign: "center", padding: 12, background: "#fdf3e4", borderRadius: 12, fontSize: 13, color: "#6a4a30", lineHeight: 2 }}>
                ...
                <motion.div key={ev.key} style={sectionCard} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  ...
                  {[
                    ...
                    <div key={d.label} style={{ display: "flex", alignItems: "flex-start", gap: 16, padding: "12px 0", borderBottom: `1px solid ${PRIMARY}22` }}>
                      ...
                      <div>
                        <div style={{ fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "#c9b090" }}>{d.label}</div>
            ...
            {sv.thank_you && (
              <motion.div style={sectionCard} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <div style={{ textAlign: "center", fontSize: 13, color: "#6a4a30", lineHeight: 2 }}>
                ...
                <div style={{ textAlign: "center", marginTop: 16 }}>
                  <div style={{ fontSize: 11, color: "#c9b090" }}>With all our love,</div>
            ...
            <div style={{ padding: "2rem 1.5rem", textAlign: "center", background: "#fff" }}>
              ...
              <div style={{ fontSize: 9, letterSpacing: "0.3em", textTransform: "uppercase", color: "#c9b090" }}>inviteglow.com · Digital Wedding Invitations</div>
            </div>
          </motion.div>
        )}
      </div>
