...
// ── Corporate / Office Event template ────────────────────────────────
// Backed by its OWN `events` table (separate from `couples` — see
// events_table_migration.sql), not the wedding data model. The row
// shape is defined below as `EventInvite`.
type EventInvite = {
  id: string
  slug: string
  template: string
  ...
  show_seating?: boolean | null
  contacts?: { name: string; phone: string }[] | null
  section_visibility?: { gallery?: boolean; countdown?: boolean; timeline?: boolean; seat_finder?: boolean; music?: boolean; thank_you?: boolean } | null
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {hasWishes && iconBtn(() => scrollToId('wishes'), 'Messages', <path d="M12 20.5s-7.5-4.9-9.8-9.3C.6 8 2 4.7 5.2 4a4.6 4.6 0 016.8 2.3A4.6 4.6 0 0118.8 4C22 4.7 23.4 8 21.8 11.2 19.5 15.6 12 20.5 12 20.5z" />, 'wishes')}
        {iconBtn(() => scrollToId('savethedate'), 'Event Day', <><rect x="3.5" y="5" width="17" height="16" rx="2.5" /><path d="M3.5 9.5h17M8 3v4M16 3v4" /></>, 'savedate')}
...
// ── Guest intro screen — light ivory + gold shimmer, "Dear [Name]," shown
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background: `radial-gradient(ellipse 75% 60% at 50% 25%, #fffdf9 0%, ${cream} 55%, #ece7d8 100%)`,
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.9 }}
        ...
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(1.9rem,6.5vw,2.7rem)", color: dark, lineHeight: 1.2 }}>
...
function Countdown({ targetDate, primaryLight, dark, muted }: { targetDate: string; primaryLight: string; dark: string; muted: string }) {
    ...
    <div style={{ display: "flex", justifyContent: "center", gap: 12, maxWidth: 380, margin: "0 auto" }}>
      {[["Days", t.d], ["Hours", t.h], ["Mins", t.m], ["Secs", t.s]].map(([l, v]) => (
        <div key={l} style={{ flex: 1, textAlign: "center" }}>
          <div style={{ width: 56, height: 56, borderRadius: 10, background: "#fff", border: `2px solid ${primaryLight}77`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 8px" }}>
...
function MusicPlayerUI({ title, artist, audioRef, primary, primaryLight, dark, muted }: { title: string; artist: string; audioRef: React.RefObject<HTMLAudioElement | null>; primary: string; primaryLight: string; dark: string; muted: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, background: "#f7f5ef", borderRadius: 16, padding: 16 }}>
...
function SeatFinder({ seats, primary, dark, cream, muted }: { seats: Record<string, string>; primary: string; dark: string; cream: string; muted: string }) {
  ...
  const search = () => {
    ...
    setRes(found ? `You are seated at ${seats[found]}` : "Name not found. Please contact the event coordinator.")
  }
  return (
    <div>
...
// ── Messages & Well Wishes wall (same `wishes` table/bucket used by the
// wedding templates — this is a guest message board, generic to any
// event type). ──
...
function WishLightbox({ media, index, onIndex, onClose }: {
  ...
  return (
    <div onClick={onClose} style={{
      position: "fixed", inset: 0, background: "rgba(15,36,56,0.92)", zIndex: 500,
      ...
      {shown.map((m, idx) => {
        ...
        return (
          <div key={idx} onClick={() => onOpen(idx)} style={{
            ...
            {isMoreTile && (
              <div style={{ position: "absolute", inset: 0, background: "rgba(15,36,56,0.55)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 18, fontWeight: 700, zIndex: 2 }}>
                +{media.length - 4}
              </div>
            )}
          </div>
...
function WishesWall({ coupleId, primary, primaryLight, dark, cream, muted }: {
  ...
  return (
    <div>
      <div style={{ background: "#fff", borderRadius: 16, padding: '18px 16px', textAlign: 'left', marginBottom: 18, boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
        {done ? (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: dark }}>Thank you for your message!</div>
            ...
            }}>Leave another message</button>
          </div>
        ) : (
          <>
            <div style={{ fontSize: 13, fontWeight: 700, color: dark, marginBottom: 10 }}>Leave a Message</div>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Your name" style={inputStyle} />
            <textarea
              value={message} onChange={e => setMessage(e.target.value)} placeholder="Share your message or well wishes..." rows={3}
      ...
      {loading ? (
        <div style={{ fontSize: 12, color: muted, textAlign: 'center' }}>Loading messages...</div>
      ) : wishes.length === 0 ? (
        <div style={{ fontSize: 12, color: muted, textAlign: 'center' }}>Be the first to leave a message!</div>
      ) : (
        <div>
...
// ── Contact Numbers — Event Coordinators. Reads ONLY the flexible
// `contacts` list (Admin → Additional Contacts). bride_phone/groom_phone
// are not used here since `bride`/`groom` hold the Event Title and
// Organized-By text for this template, not a person's name. ──
...
// ── Card + section styles ──
const sectionCard: React.CSSProperties = { background: "#fff", margin: "0 16px 16px", borderRadius: 22, padding: "1.8rem", boxShadow: "0 2px 20px rgba(15,36,56,0.06)" }
...
const sectionTitle = (dark: string): React.CSSProperties => ({ fontFamily: "'Playfair Display',serif", fontSize: "1.5rem", color: dark, textAlign: "center", marginBottom: 20 })

export default function CorporateEventTemplate({ couple }: { couple: EventInvite }) {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", background: "#f7f5ef" }} />}>
      <CorporateEventInner couple={couple} />
    </Suspense>
  )
}

function CorporateEventInner({ couple }: { couple: EventInvite }) {
  ...
  // `events` row.
  type RenderableEvent = { key: 'main'; label: string; icon: string; venue: string; venue_address: string; date: string; maps_url: string }
  const eventsList: RenderableEvent[] = couple.event_date
    ? [{ key: 'main', label: 'The Event', icon: '🎉', venue: couple.venue || '', venue_address: couple.venue_address || '', date: couple.event_date, maps_url: couple.maps_url || '' }]
    : []
  ...
  const W = {
    title: couple.title, host: couple.host || '',
    date: couple.event_date, coverPhoto: couple.cover_photo || '',
    song: couple.song_title || 'Background Music', artist: couple.song_artist || 'InviteGlow',
    timeline: couple.timeline || [], seats: couple.seats || {}, gallery: couple.gallery || [],
  }

  // Event Coordinators — the flexible `contacts` list is the only source
  // here (there's no bride_phone/groom_phone equivalent on this table).
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
            ...
            {sv.countdown && (
              <div id="savethedate" style={{ ...sectionCard, textAlign: "center" }}>
                <div style={sectionEyebrow(PRIMARY)}>Counting Down to the Event</div>
            ...
            {hasWishes && (
              <div id="wishes" style={sectionCard}>
                ...
                <div style={sectionTitle(DARK)}>Messages & Well Wishes</div>
            ...
            {contactList.length > 0 && (
              <div id="contact" style={sectionCard}>
                ...
                <div style={sectionTitle(DARK)}>Event Coordinators</div>
            ...
            {sv.thank_you && (
              <motion.div style={sectionCard} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <div style={{ textAlign: "center", fontSize: 13, color: DARK, lineHeight: 2 }}>
                  {(couple as any).thank_you_text || "We're delighted to have you join us for this occasion. Thank you for being part of our journey and for making this event even more special."}
                </div>
                <div style={{ textAlign: "center", marginTop: 16 }}>
                  <div style={{ fontSize: 11, color: MUTED }}>With appreciation,</div>
                  <div style={{ fontFamily: "'Playfair Display',serif", fontSize: "1.4rem", color: PRIMARY, marginTop: 4 }}>{W.title}</div>
                </div>
              </motion.div>
            )}
