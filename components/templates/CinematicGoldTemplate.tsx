...
function ScratchReveal({
  photo, bride, groom, dateDisplay, venue, onComplete,
}: { photo: string; bride: string; groom: string; dateDisplay: string; venue: string; onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      grad.addColorStop(0.3, '#b8862e')
      grad.addColorStop(0.5, '#e8c468')
      grad.addColorStop(0.7, '#9a6a1e')
      grad.addColorStop(1, '#d4a84a')
      ctx.fillStyle = grad
    ...
    <div ref={containerRef} style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden", cursor: revealed ? "default" : "pointer" }}>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0 1.5rem 36px", textAlign: "center", zIndex: 2 }}>
        ...
        <div style={{ fontFamily: "'Cormorant Garamond',serif", fontWeight: 300, fontStyle: "italic", fontSize: "clamp(2.2rem,8vw,3.4rem)", color: "#fff", lineHeight: 1.05, textShadow: "0 4px 30px rgba(0,0,0,0.5)" }}>
          {bride}<span style={{ color: "#e8c468" }}> &amp; </span>{groom}
        </div>
    ...
    <div style={{ display: "flex", maxWidth: 360, margin: "0 auto" }}>
      {[["Days", t.d], ["Hours", t.h], ["Mins", t.m], ["Secs", t.s]].map(([l, v]) => (
        <div key={l} style={{ flex: 1, textAlign: "center", padding: "16px 8px" }}>
          <div style={{ width: 56, height: 56, borderRadius: "50%", background: "linear-gradient(135deg,#1a1208,#2d2008)", border: "1px solid rgba(232,196,104,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 8px" }}>
            <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.3rem", color: "#e8c468", fontWeight: 600 }}>{v}</span>
          </div>
          <span style={{ fontSize: 8, letterSpacing: "0.25em", textTransform: "uppercase", color: "rgba(232,196,104,0.5)" }}>{l}</span>
        </div>
      ))}
    </div>
  )
    ...
    <div style={{ position: "fixed", inset: 0, pointerEvents: "none", overflow: "hidden", zIndex: 1 }}>
      {items.map(p => (
        <div key={p.id} style={{
          ...
          background: "radial-gradient(circle, #e8c468, transparent)", opacity: 0.7,
...
function MusicPlayerUI({ title, artist, audioRef }: { title: string; artist: string; audioRef: React.RefObject<HTMLAudioElement | null> }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, background: "#1a1208", borderRadius: 14, padding: 16, border: "1px solid rgba(232,196,104,0.15)" }}>
      <div style={{ width: 44, height: 44, borderRadius: playing ? "50%" : 10, background: "linear-gradient(135deg,#e8c468,#9a6a1e)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0, animation: playing ? "spin 4s linear infinite" : "none" }}>🎵</div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 14, fontWeight: 600, color: "#fff" }}>{title}</div>
        <div style={{ fontSize: 11, color: "rgba(232,196,104,0.5)", marginTop: 2 }}>{artist}</div>
        <div style={{ height: 3, background: "rgba(232,196,104,0.1)", borderRadius: 100, marginTop: 8 }}>
          <div style={{ height: "100%", width: `${prog}%`, background: "linear-gradient(to right,#e8c468,#9a6a1e)", borderRadius: 100, transition: "width 0.3s" }} />
        </div>
      </div>
      <button onClick={toggle} style={{ width: 38, height: 38, borderRadius: "50%", background: "#e8c468", border: "none", color: "#1a1208", cursor: "pointer", fontSize: 13, flexShrink: 0, fontWeight: 700 }}>
        {playing ? "⏸" : "▶"}
      </button>
    </div>
  )
...
function RSVP({ coupleId, askDrinking }: { coupleId: string; askDrinking: boolean }) {
  ...
  const inputStyle: React.CSSProperties = { width: "100%", padding: "13px 16px", borderRadius: 10, border: "1px solid rgba(232,196,104,0.2)", background: "#241a0c", color: "#fff", fontSize: 14, outline: "none", marginBottom: 12, fontFamily: "'Inter',sans-serif" }

  return (
    <div style={{ background: "#1a1208", padding: "40px 1.5rem", textAlign: "center" }}>
      <div style={{ fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", color: "#e8c468", marginBottom: 8, fontWeight: 600 }}>Be Our Guest</div>
      ...
      <div style={{ background: "#241a0c", borderRadius: 16, padding: 24, maxWidth: 380, margin: "0 auto", border: "1px solid rgba(232,196,104,0.1)" }}>

        {step === "form" && (
          <>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Your full name" style={inputStyle} />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <button onClick={handleAccept} style={{ padding: 13, borderRadius: 10, background: "linear-gradient(135deg,#e8c468,#9a6a1e)", color: "#1a1208", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>✓ Accept</button>
        ...
        {step === "count" && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, marginBottom: 16 }}>
              <button onClick={() => setGuestCount(c => Math.max(1, c - 1))} style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(232,196,104,0.1)", color: "#e8c468", border: "none", cursor: "pointer", fontSize: 16 }}>−</button>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "1.8rem", color: "#fff", minWidth: 40 }}>{guestCount}</div>
              <button onClick={() => setGuestCount(c => Math.min(20, c + 1))} style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(232,196,104,0.1)", color: "#e8c468", border: "none", cursor: "pointer", fontSize: 16 }}>+</button>
            </div>
            <button onClick={handleCountNext} disabled={saving} style={{ width: "100%", padding: 13, borderRadius: 10, background: "linear-gradient(135deg,#e8c468,#9a6a1e)", color: "#1a1208", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, opacity: saving ? 0.6 : 1 }}>
              {saving ? "..." : "Continue →"}
            </button>
          </motion.div>
        )}
        ...
        {step === "drinking" && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <button onClick={() => save("yes", "yes", guestCount)} disabled={saving} style={{ padding: 13, borderRadius: 10, background: "rgba(232,196,104,0.1)", color: "#e8c468", border: "1px solid rgba(232,196,104,0.3)", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>🍷 Yes</button>
              <button onClick={() => save("yes", "no", guestCount)} disabled={saving} style={{ padding: 13, borderRadius: 10, background: "rgba(232,196,104,0.1)", color: "#e8c468", border: "1px solid rgba(232,196,104,0.3)", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>🥤 No</button>
            </div>
          </motion.div>
        )}
        ...
        {step === "done" && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic", fontSize: "1.3rem", color: "#e8c468", marginBottom: 4 }}>
    ...
    <div>
      <div style={{ display: "flex", gap: 10 }}>
        <input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => e.key === "Enter" && search()}
          placeholder="Enter your name..." style={{ flex: 1, padding: "13px 16px", borderRadius: 10, border: "1px solid rgba(232,196,104,0.15)", background: "#1a1208", color: "#fff", fontSize: 14, outline: "none", fontFamily: "'Inter',sans-serif" }} />
        <button onClick={search} style={{ padding: "13px 20px", borderRadius: 10, background: "linear-gradient(135deg,#e8c468,#9a6a1e)", color: "#1a1208", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700 }}>Search</button>
      </div>
      {res && <div style={{ marginTop: 12, fontSize: 14, color: res.startsWith("You") ? "#e8c468" : "rgba(255,255,255,0.5)", fontWeight: res.startsWith("You") ? 600 : 400 }}>{res}</div>}
    </div>
  )
}
...
export default function CinematicGoldTemplate({ couple }: { couple: Couple }) {
  ...
  return (
    <div style={{ fontFamily: "'Inter',sans-serif", minHeight: "100vh", background: "#0d0905" }}>
      <style>{`
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", background: "#1a1208", boxShadow: "0 0 100px rgba(0,0,0,0.5)", position: "relative", overflow: "hidden" }}>
                ...
                <motion.div
                  ...
                  <motion.button
                    ...
                    style={{
                      ...
                      background: "linear-gradient(135deg,#e8c468,#9a6a1e)", color: "#1a1208",
                      fontSize: 11, letterSpacing: "0.22em", textTransform: "uppercase", cursor: "pointer",
                      fontFamily: "'Inter',sans-serif", fontWeight: 700,
                      boxShadow: "0 8px 30px rgba(232,196,104,0.4)",
                    }}>
                    Enter Invitation →
                  </motion.button>
        ...
        {opened && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }} style={{ position: "relative" }}>
            ...
            <div style={{ position: "relative", height: 380, overflow: "hidden" }}>
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0 1.5rem 28px", textAlign: "center", zIndex: 4 }}>
                ...
                <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 14 }}>
                  <a href="#rsvp" style={{ background: "linear-gradient(135deg,#e8c468,#9a6a1e)", color: "#1a1208", borderRadius: 100, padding: "10px 20px", fontSize: 11, letterSpacing: "0.1em", textDecoration: "none", fontWeight: 700 }}>RSVP</a>
            ...
            <div style={{ padding: "2rem 1.5rem", textAlign: "center", background: "#0d0905" }}>
              <div style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic", fontSize: "1.3rem", color: "#e8c468", marginBottom: 4 }}>InviteGlow</div>
