      ...
      {shown.map((m, idx) => {
        ...
        return (
          <div key={idx} onClick={() => onOpen(idx)} style={{
            ...
            {isMoreTile && (
              <div style={{ position: "absolute", inset: 0, background: "rgba(20,8,15,0.55)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 18, fontWeight: 700, zIndex: 2 }}>
                +{media.length - 4}
              </div>
            )}
          </div>
...
function WishesWall({ coupleId, primary, primaryLight, dark, cream, muted }: {
  ...
  return (
    <div>
      <div style={{ background: cream, borderRadius: 16, padding: '18px 16px', textAlign: 'left', marginBottom: 18, border: `1px solid ${primaryLight}55` }}>
        {done ? (
          ...
          <>
            ...
            <input
              ...
              style={{ width: '100%', padding: '10px 13px', borderRadius: 10, border: `1px solid ${primaryLight}`, fontSize: 13, outline: 'none', marginBottom: 10, boxSizing: 'border-box', fontFamily: "'Inter',sans-serif", background: '#fff' }}
            />
            <textarea
              value={message} onChange={e => setMessage(e.target.value)} placeholder="Write your wishes for the couple..." rows={3}
              style={{ width: '100%', padding: '10px 13px', borderRadius: 10, border: `1px solid ${primaryLight}`, fontSize: 13, outline: 'none', marginBottom: 10, boxSizing: 'border-box', fontFamily: "'Inter',sans-serif", resize: 'vertical', background: '#fff' }}
            />
            <label style={{
            ...
            <button onClick={submit} disabled={submitting} style={{
              ...
              background: `linear-gradient(135deg,${primary},${primaryLight})`, color: '#fff', fontWeight: 700, fontSize: 13, fontFamily: "'Inter',sans-serif", opacity: submitting ? 0.6 : 1,
      ...
      {loading ? (
        ...
        <div>
          ...
          {wishes.slice(page * PER_PAGE, page * PER_PAGE + PER_PAGE).map((w, i, arr) => {
            ...
            return (
              <div key={w.id} style={{ padding: '12px 0', borderBottom: i < arr.length - 1 ? `1px solid ${primaryLight}55` : 'none', textAlign: 'left' }}>
          ...
          {wishes.length > PER_PAGE && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginTop: 16, paddingTop: 14, borderTop: `1px solid ${primaryLight}55`, flexWrap: 'wrap' }}>
                  ...
                  {[
                    ...
                    <div key={d.label} style={{ display: "flex", alignItems: "flex-start", gap: 16, padding: "12px 0", borderBottom: `1px solid ${PRIMARY_LIGHT}33` }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 15, color: DARK, fontWeight: 700, marginTop: 2, fontFamily: "'Inter',sans-serif" }}>{d.val}</div>
                        {d.sub && <div style={{ fontSize: 12, color: MUTED, marginTop: 2 }}>{d.sub}</div>}
                      </div>
                    </div>
                  ))}
                  {(ev.maps_url || ev.venue || ev.venue_address) && (
                    <a href={normalizeMapsUrl(ev.maps_url, ev.venue, ev.venue_address)} target="_blank" rel="noopener noreferrer"
