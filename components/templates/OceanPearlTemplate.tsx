  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background: `linear-gradient(160deg, ${cream} 0%, #e0f2f7 45%, ${cream} 100%)`,
...
function MusicPlayerUI({ title, artist, audioRef, primary, primaryLight, dark, muted }: { title: string; artist: string; audioRef: React.RefObject<HTMLAudioElement | null>; primary: string; primaryLight: string; dark: string; muted: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, background: "#f0f9fb", borderRadius: 16, padding: 16 }}>
...
function RSVP({ coupleId, askDrinking, primary, dark, cream, muted, guestName }: { coupleId: string; askDrinking: boolean; primary: string; dark: string; cream: string; muted: string; guestName: string }) {
  ...
  return (
    <div style={{ background: `linear-gradient(135deg,#d0eef5,${cream})`, padding: "40px 1.5rem", textAlign: "center" }}>
        ...
        {step === "form" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <button onClick={() => name.trim() && save("no", null, 1)} disabled={saving} style={{ padding: 13, borderRadius: 10, background: "transparent", color: muted, border: "1px solid #d0e8ee", cursor: "pointer", fontSize: 12, opacity: saving ? 0.6 : 1 }}>{saving ? "..." : "✗ Decline"}</button>
            </div>
          </>
        )}
        {step === "count" && (
...
function WishLightbox({ media, index, onIndex, onClose }: {
  ...
  return (
    <div onClick={onClose} style={{
      position: "fixed", inset: 0, background: "rgba(13,46,58,0.92)", zIndex: 500,
      ...
      {shown.map((m, idx) => {
        ...
        return (
          <div key={idx} onClick={() => onOpen(idx)} style={{
            ...
            {isMoreTile && (
              <div style={{ position: "absolute", inset: 0, background: "rgba(13,46,58,0.55)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 18, fontWeight: 700, zIndex: 2 }}>
                +{media.length - 4}
              </div>
            )}
          </div>
...
function ContactRow({ name, phone, primary }: { name: string; phone: string; primary: string }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, background: '#f0f9fb', borderRadius: 12, padding: '12px 16px' }}>
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#0d2e3a' }}>{name}</div>
        <div style={{ fontSize: 12.5, color: '#6a98a8', marginTop: 2 }}>{phone}</div>
      </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
            ...
            {sv.thank_you && (
              <motion.div style={sectionCard} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <div style={{ textAlign: "center", fontSize: 13, color: "#2a5060", lineHeight: 2 }}>
                ...
                <div style={{ textAlign: "center", marginTop: 16 }}>
                  <div style={{ fontSize: 11, color: "#9cc4d0" }}>With all our love,</div>
