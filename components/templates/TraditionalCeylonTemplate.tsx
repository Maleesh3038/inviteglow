...
function MusicPlayerUI({ title, artist, audioRef, primary, primaryLight, dark, muted }: { title: string; artist: string; audioRef: React.RefObject<HTMLAudioElement | null>; primary: string; primaryLight: string; dark: string; muted: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14, background: "#fbf6e9", borderRadius: 16, padding: 16 }}>
      <div style={{ width: 46, height: 46, borderRadius: "50%", background: `linear-gradient(135deg,${primaryLight},#8a7420)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, flexShrink: 0, animation: playing ? "spin 4s linear infinite" : "none" }}>🎵</div>
      <div style={{ flex: 1 }}>
...
function RSVP({ coupleId, askDrinking, primary, dark, cream, muted, guestName }: { coupleId: string; askDrinking: boolean; primary: string; dark: string; cream: string; muted: string; guestName: string }) {
  ...
  return (
    <div style={{ background: `linear-gradient(135deg,#e4dfc0,${cream})`, padding: "40px 1.5rem", textAlign: "center" }}>
        ...
        {step === "form" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              ...
              <button onClick={() => name.trim() && save("no", null, 1)} disabled={saving} style={{ padding: 13, borderRadius: 10, background: "transparent", color: muted, border: "1px solid #e4dfc0", cursor: "pointer", fontSize: 12, opacity: saving ? 0.6 : 1 }}>{saving ? "..." : "✗ Decline"}</button>
            </div>
          </>
        )}
        {step === "count" && (
...
export default function TraditionalCeylonTemplate({ couple }: { couple: Couple }) {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", background: "#fbf6e9" }} />}>
      <TraditionalCeylonInner couple={couple} />
    </Suspense>
  )
}

function TraditionalCeylonInner({ couple }: { couple: Couple }) {
  ...
  return (
    <div style={{ fontFamily: "'Inter',sans-serif", minHeight: "100vh", background: "#fbf6e9" }}>
      <style>{`
        ...
        input::placeholder { color: #b0b898; }
      `}</style>
        ...
        <AnimatePresence>
          {!opened && (
            <motion.div key="cover" exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.5 }}
              ...
              <div style={{ position: "absolute", inset: 0, background: `linear-gradient(180deg, rgba(31,46,34,0.45) 0%, rgba(31,46,34,0.15) 35%, rgba(31,46,34,0.3) 65%, rgba(31,46,34,0.72) 100%)` }} />
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}
                ...
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.15)", backdropFilter: "blur(6px)", borderRadius: 100, padding: "6px 14px", fontSize: 10, letterSpacing: "0.25em", textTransform: "uppercase", color: "#fff", marginBottom: "1.2rem", border: "1px solid rgba(255,255,255,0.25)" }}>
                  {(couple as any).cover_badge_text || 'A Traditional Union'}
                </div>
                ...
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.85)", lineHeight: 1.7, margin: "1.2rem 0 1.6rem", textShadow: "0 2px 10px rgba(0,0,0,0.4)" }}>
                  Join us as we honour tradition and<br />begin our forever together
                </div>

                <button onClick={handleOpen} style={{
                  display: "inline-flex", alignItems: "center", gap: 10, background: `linear-gradient(135deg,${PRIMARY_LIGHT},#8a7420)`, color: "#1f2e22",
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
            ...
            {(W.brideFamilyName || W.groomFamilyName) && (
              <motion.div style={sectionCard} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <div style={{ textAlign: "center", padding: 12, background: "#f2eed2", borderRadius: 12, fontSize: 13, color: "#2f4a35", lineHeight: 2 }}>
            ...
            {sv.thank_you && (
              <motion.div style={sectionCard} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <div style={{ textAlign: "center", fontSize: 13, color: "#2f4a35", lineHeight: 2 }}>
                ...
                <div style={{ textAlign: "center", marginTop: 16 }}>
                  <div style={{ fontSize: 11, color: "#b0b898" }}>With all our love,</div>
