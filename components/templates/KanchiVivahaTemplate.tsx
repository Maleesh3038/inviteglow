...
// No default photo fallback for Kanchi Vivaha — if a couple has no photo
...
// this exact path (videos/kanchi-vivaha-cover.mp4) so this default resolves.
// Admins can still override per-couple via the "Cover Video URL" field.
const DEFAULT_COVER_VIDEO = "https://eqacrwhbrfqcnlgegvtl.supabase.co/storage/v1/object/public/wedding-photos/videos/kanchi-vivaha-cover.mp4"
const DEFAULT_SONG_URL = "/audio/calm-wedding.mp3"
const DEFAULT_SONG_TITLE = "A Sacred Union"
...
// ── Diya divider — a small oil lamp with flame flanked by thin flourish
// lines, the auspicious lamp-lighting motif central to Tamil weddings. ──
function LaurelDivider({ color, size = 20 }: { color: string; size?: number }) {
  const flourish = (flip: boolean) => (
    <svg width={size * 1.5} height={size} viewBox="0 0 40 24" fill="none" style={flip ? { transform: "scaleX(-1)" } : undefined}>
      ...
      <path d="M22 12c3-3 5-2 6 0-1 2-3 3-6 0z" stroke={color} strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
      ...
      <svg width={size * 0.62} height={size * 0.62} viewBox="0 0 24 24" fill={color}>
        <path d="M12 3.5c-2 3-2 5.2 0 7.2 2-2 2-4.2 0-7.2z" />
        <path d="M2.5 14c2.5 3.5 6 5 9.5 5s7-1.5 9.5-5c-2.5-1.6-6-2.4-9.5-2.4S5 12.4 2.5 14z" />
        <ellipse cx="12" cy="14" rx="2.6" ry="1.3" fill="none" stroke={color} strokeWidth="0.9" opacity="0.6" />
      </svg>
      {flourish(true)}
    </div>
  )
...
function RSVP({ coupleId, askDrinking, primary, dark, cream, muted, guestName }: { coupleId: string; askDrinking: boolean; primary: string; dark: string; cream: string; muted: string; guestName: string }) {
  ...
  return (
    <div style={{ padding: "0 1.5rem 2.4rem", textAlign: "center" }}>
      ...
      <div style={{ fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", color: primary, margin: "16px 0 8px", fontWeight: 700, fontFamily: "'Oswald',sans-serif" }}>Mangala RSVP</div>
      <div style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic", fontSize: "1.8rem", color: dark, marginBottom: 24 }}>Will You Grace Our Union?</div>
...
export default function KanchiVivahaTemplate({ couple }: { couple: Couple }) {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", background: "#faf8f2" }} />}>
      <KanchiVivahaInner couple={couple} />
    </Suspense>
  )
}

function KanchiVivahaInner({ couple }: { couple: Couple }) {
  ...
  // Kanchi Vivaha video quietly overriding it.
            ...
            <motion.div key="cover" exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.5 }}
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}
                ...
                <div style={{ margin: "10px 0", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
                  ...
                  <svg width={13} height={13} viewBox="0 0 24 24" fill={GOLD}><path d="M12 3.5c-2 3-2 5.2 0 7.2 2-2 2-4.2 0-7.2zM2.5 14c2.5 3.5 6 5 9.5 5s7-1.5 9.5-5c-2.5-1.6-6-2.4-9.5-2.4S5 12.4 2.5 14z" /></svg>
                  <div style={{ height: 1, width: 40, background: "rgba(255,255,255,0.6)" }} />
                </div>
                ...
                <div style={{
                  ...
                  {couple.intro_text || "Woven together like silk and gold, our forever begins today."}
                </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
            ...
            <motion.div style={cardStyle()} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              <div style={{ textAlign: "center", fontSize: 13, color: DARK, lineHeight: 2, fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic" }}>
                {(couple as any).family_invitation_text ||
                  "With the blessings of our elders and the warmth of tradition, our two families come together in joy. We invite you to witness the sacred union of our hearts."}
              </div>
            </motion.div>
            ...
            {eventsList.map(ev => {
              ...
              return (
                <motion.div key={ev.key} style={cardStyle()} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <CornerBrackets color={GOLD} />
                  <div style={pretitleStyle(PRIMARY)}>{ev.icon} Mangala Vidhi</div>
                  ...
                  {[
                    ...
                    ...(ev.dressCode ? [{ icon: "👗", label: "Dress Code", val: ev.dressCode }] : []),
                  ].map(d => (
            ...
            {sv.timeline && W.timeline.length > 0 && (
              <motion.div style={cardStyle()} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                ...
                <div style={titleStyle(DARK)}>Wedding Day Schedule</div>
            ...
            <div id={contactList.length > 0 ? undefined : "contact"} style={{ padding: "2rem 1.5rem 6rem", textAlign: "center", background: "#fff", borderTop: `1px solid ${GOLD}55`, borderRadius: "24px 24px 0 0" }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 10 }}>
                <svg width={30} height={30} viewBox="0 0 24 24" fill={GOLD}><path d="M12 3.5c-2 3-2 5.2 0 7.2 2-2 2-4.2 0-7.2zM2.5 14c2.5 3.5 6 5 9.5 5s7-1.5 9.5-5c-2.5-1.6-6-2.4-9.5-2.4S5 12.4 2.5 14z" /></svg>
              </div>
