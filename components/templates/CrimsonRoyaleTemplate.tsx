...
// No default photo fallback for Crimson Royale — if a couple has no photo
...
// this exact path (videos/crimson-royale-cover.mp4) so this default resolves.
// Admins can still override per-couple via the "Cover Video URL" field.
const DEFAULT_COVER_VIDEO = "https://eqacrwhbrfqcnlgegvtl.supabase.co/storage/v1/object/public/wedding-photos/videos/crimson-royale-cover.mp4"
const DEFAULT_SONG_URL = "/audio/calm-wedding.mp3"
const DEFAULT_SONG_TITLE = "A Vow of Devotion"
...
// ── Crest divider — flourish lines flanking a small gold crown, the
// regal equivalent of a leaf/laurel divider used in other templates. ──
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
      ...
      <svg width={size * 0.62} height={size * 0.62} viewBox="0 0 24 24" fill={color}>
        <path d="M4.5 17L3 8l4.6 3.4L12 5l4.4 6.4L21 8l-1.5 9z" />
        <path d="M3.2 19h17.6v1.8a1 1 0 01-1 1H4.2a1 1 0 01-1-1z" />
...
function RSVP({ coupleId, askDrinking, primary, dark, cream, muted, guestName }: { coupleId: string; askDrinking: boolean; primary: string; dark: string; cream: string; muted: string; guestName: string }) {
  ...
  return (
    <div style={{ padding: "0 1.5rem 2.4rem", textAlign: "center" }}>
      ...
      <div style={{ fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", color: primary, margin: "16px 0 8px", fontWeight: 700, fontFamily: "'Oswald',sans-serif" }}>Royal RSVP</div>
      <div style={{ fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic", fontSize: "1.8rem", color: dark, marginBottom: 24 }}>Will You Join Our Celebration?</div>
...
export default function CrimsonRoyaleTemplate({ couple }: { couple: Couple }) {
  return (
    <Suspense fallback={<div style={{ minHeight: "100vh", background: "#faf8f2" }} />}>
      <CrimsonRoyaleInner couple={couple} />
    </Suspense>
  )
}

function CrimsonRoyaleInner({ couple }: { couple: Couple }) {
  ...
  // Crimson Royale video quietly overriding it.
            ...
            <motion.div key="cover" exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.5 }}
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}
                ...
                <div style={{
                  ...
                  {couple.intro_text || "With love as our crown, we begin our forever."}
                </div>
              ...
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "2rem 1.5rem", textAlign: "center", zIndex: 5 }}>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 14 }}>
                    <a href="#contact" style={{ background: `linear-gradient(135deg,${PRIMARY},${PRIMARY_LIGHT})`, color: "#fff", borderRadius: 100, padding: "10px 22px", fontSize: 11, letterSpacing: "0.15em", textDecoration: "none", fontFamily: "'Oswald',sans-serif" }}>Contact</a>
            ...
            <motion.div style={cardStyle()} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              <div style={{ textAlign: "center", fontSize: 13, color: DARK, lineHeight: 2, fontFamily: "'Cormorant Garamond',serif", fontStyle: "italic" }}>
                {(couple as any).family_invitation_text ||
                  "Like a story written for the ages, our love has grown richer with every season. With joy in our hearts, we invite you to witness the union of two souls committed for life."}
              </div>
            </motion.div>
            ...
            {eventsList.map(ev => {
              ...
              return (
                <motion.div key={ev.key} style={cardStyle()} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                  <CornerBrackets color={GOLD} />
                  <div style={pretitleStyle(PRIMARY)}>{ev.icon} Order of Celebration</div>
